...
function buildEchangeCharges(params){
  ...
  const noVat = (sub === 'transit' || sub === 'tbl');   // transit & TBL : pas de TVA
  const lines = [];
  ...
  template.forEach(function(ch){
    ...
    if((isPort || isMun || isCom) && commodities.length > 0){
      ...
      commodities.forEach(function(cm){
        ...
        lines.push({code:code, desc:ch.d+' ('+commName+')', bit:'COFR', qty:w, rate:rate, unit:'TO', vat:(noVat?0:ch.vat), redevance:true});
      });
    } else {
      // Standard charge
      let qty;
      ...
      lines.push({code:code, desc:ch.d, bit:'COFR', qty:qty, rate:ch.rate, unit:(ch.base==='TO'?'TO':'EA'), vat:(noVat?0:ch.vat), redevance:isRedevance});
    }
  });
  return lines;
}
